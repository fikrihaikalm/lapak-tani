...
@section('title', 'Mengikuti ' . $user->name . ' - Lapak Tani')
    ...
    <div class="mb-8">
        <div class="flex items-center space-x-4 mb-4">
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ $user->name }} Mengikuti</h1>
                    <p class="text-gray-600">{{ $following->total() }} mengikuti</p>
                </div>
            </div>
        </div>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h3 class="text-lg font-medium text-gray-900">
                                    <a href="{{ route('social.profile', $followedUser->id) }}" class="hover:text-hijau-600">
                                        {{ $followedUser->name }}
                                    </a>
                                </h3>
                                @if($followedUser->farm_name)
                                    <p class="text-sm text-gray-500">{{ $followedUser->farm_name }}</p>
                                @endif
                                <p class="text-xs text-gray-400">
                                    Diikuti sejak {{ $followedUser->pivot->created_at->diffForHumans() }}
                                </p>
                            </div>
                        </div>
    ...
    @else
        ...
        <div class="text-center py-16">
            <div class="max-w-md mx-auto">
                ...
                <h2 class="text-2xl font-semibold text-gray-900 mb-4">Tidak Mengikuti Siapa Pun</h2>
                <p class="text-gray-600">{{ $user->name }} belum mengikuti pengguna lain.</p>
            </div>
        </div>
    @endif
</div>
